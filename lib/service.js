...
function Service(opts, req, res) {
  ...
  self.cwd = opts.cwd;
  self.transform = opts.transform ? opts.transform(self.repo) : null;
  ...
  if (decoder) {
    if (self.transform) {
      req.pipe(decoder()).pipe(through(self.transform)).pipe(ts).pipe(buffered);
    } else {
      req.pipe(decoder()).pipe(ts).pipe(buffered);
    }
  } else {
    if (self.transform) {
      req.pipe(ts).pipe(through(self.transform)).pipe(buffered);
    } else {
